 ...
 * @author ACER
 */
public class SearchByCategoryAndRentalDateServlet extends HttpServlet {

    private final String MAIN_PAGE = "main.jsp";

    private int getMaxRecord(int categoryID, long year) throws SQLException, NamingException {
        TblCarDAO dao = new TblCarDAO();
        return dao.countLikeCategoryAndYear(categoryID, year);
    }

    private int getMaxPage(int categoryID, long year) {
        int maxRecord = 0;
        int maxPage = 0;
        try {
            maxRecord = getMaxRecord(categoryID, year);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            long year = Long.parseLong(request.getParameter("txtYear"));
            //Paging
            int currentPage = 1;
            //max page
            int maxPage = getMaxPage(categoryID, year);
            //page
            ...
            //search result
            TblCarDAO dao = new TblCarDAO();
            List<TblCarDTO> list = dao.searchLikeCategoryAndYear(categoryID, year, page);
            if (list != null) {
