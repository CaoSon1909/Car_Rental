 ...
 * @author ACER
 */
public class ShowFeedbackPageServlet extends HttpServlet {

    private final String FEED_BACK_PAGE = "feedback.jsp";

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setContentType("text/html;charset=UTF-8");
        PrintWriter out = response.getWriter();
        String url = FEED_BACK_PAGE;
        
        String carIDString = request.getParameter("txtCarID");
        String email = request.getParameter("txtEmail");
        String orderIDString = request.getParameter("txtOrderID");

        request.setAttribute("CAR_ID", carIDString);
